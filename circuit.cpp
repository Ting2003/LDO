...
void Circuit::compute_ldo_current(){
	...
	for(size_t i=0;i<ldolist.size();i++){
		...
		current = (VDD_G - nA->value)/resistor_val;
		current_sum += current;
		net = nA->nbr[d];
		if(net == NULL)
			clog<<"null net; "<<endl;*/
		...
		//for(DIRECTION d = WEST; d <= BOTTOM; d = (DIRECTION)(d+1)){
			...
			current = (nb->value - nA->value)/0.5;//net->value;
			current_sum += current;
...
void Circuit::rebuild_voltage_nets_l(vector<Pad*>pad_set, vector<Node*> pad_set_best){
	...
	for(size_t i=0;i<pad_set.size();i++){
		...
		//clog<<"i, rm_node, add_node: "<<i<<" "<<*rm_node<<" "<<*pad_set_best[i]<<endl;
		bool flag = false;
		size_t j=0;
		for(j=0;j<pad_set_best.size();j++){
			Net *net = pad_set_best[j]->nbr[TOP];
			if(net == NULL) continue;
			if(net == NULL) clog<<"Null net. "<<endl;
			//clog<<"net: "<<*net<<endl;
			nb = net->ab[0];
			if(nb->name != rm_node->name)
				nb = net->ab[1];
			if(nb->name == rm_node->name){
				flag = true;
				break;
	...
	for(size_t i=0;i<pad_set.size();i++){
		//clog<<endl<<"i of "<<i<<" "<<pad_set.size()<<endl;
		// skip preserved pad
		...
		add_node = get_node_pt(map_node_pt_g, 
			sstream.str());
		// clog<<"i, rm_nod, add_node, na: "<<i<<" "<<*rm_node<<" "<<*add_node<<" "<<*na<<endl;
		...
		add_node->nbr[TOP] = add_resistor_net;
		add_node_new->nbr[BOTTOM] = add_resistor_net;
		add_node_new->nbr[TOP] = add_net;
		//if(rm_node->name =="n6_75_172"){
			/*clog<<"rm_node, add_node: "<<*rm_node<<" "<<*add_node_new<<endl;
			clog<<"ad_node top: "<<*add_node->nbr[TOP]<<endl;
			clog<<"X node bot, top: "<<*add_node_new->nbr[BOTTOM]<<" "<<*add_node_new->nbr[TOP]<<endl;			*/
		//}
...
void Circuit::rebuild_voltage_nets(vector<Pad*>&pad_set, vector<Node*> &origin_pad_set, bool local_flag){
	...
	for(size_t i=0;i<origin_pad_set.size();i++){
		...
		add_node_new->nbr[TOP] = add_net;
		//if(rm_node->name =="n6_75_172"){
			/*cout<<"rm_node, add_node: "<<*rm_node<<" "<<*add_node_new<<endl;
			cout<<"ad_node top: "<<*add_node->nbr[TOP]<<endl;
			cout<<"X node bot, top: "<<*add_node_new->nbr[BOTTOM]<<" "<<*add_node_new->nbr[TOP]<<endl;
			*/
		//}
...
void Circuit::recover_local_pad(Tran &tran, vector<LDO*> &ldolist_best){
	...
	Node *nd_new = NULL;
	origin_pad_set_l.clear();
	//origin_pad_set_l.resize(ldolist.size());
	...
	for(size_t i=0;i<ldolist.size();i++){
		...
		nd_new = nd_new->rep;	
		//clog<<"nd_new: "<<*nd_new<<endl;
		//clog<<"i, origin pad set_l: "<<i<<" "<<*origin_pad_set_l[i]<<endl;
		//origin_pad_set_l[i] = nd_new;
		origin_pad_set_l.push_back(nd_new);
		//clog<<"add, rm: "<<*nd_new<<" "<<*origin_pad_set_l[i]<<endl;
	}
	...
	for(size_t i=0;i<ldolist.size();i++){
		sstream.str("");
		sstream<<"n"<<ldolist[i]->A->pt.z<<"_"<<ldolist_best[i]->node[0]->x<<"_"<<ldolist_best[i]->node[0]->y;
		Node *nA = get_node_pt(map_node_pt_g, sstream.str());
		if(nA == NULL) {
			clog<<"null node. "<<sstream.str()<<endl;
			continue;
		}
		ldolist[i]->A = nA;
